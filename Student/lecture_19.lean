...
example : ∀ (n : ℕ), True := fun n => True.intro --for all nats, there are worlds in which it is true (I think that's what this means)
#check ∀ (n : Nat), True
-- a proof of forall proposition is really just a proof that for any arbitrarily selected element of the quantified type, there is proof of the predicate applied to that object.
...
-- there exists a natural number 'n' such that n%2 = 0.
